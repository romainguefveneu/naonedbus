...
DELETE FROM favoris WHERE _id IN (SELECT favoris._id FROM favoris LEFT JOIN arrets ON arrets.code = favoris.codeArret AND arrets.codeSens = arrets.codeSens AND favoris.codeLigne = arrets.codeLigne WHERE arrets.code IS NULL);

...
public class StopBookmarkProvider extends CustomContentProvider {
	...
	private static final String AUTHORITY = "net.naonedbus.provider.StopBookmarkProvider";
	...
	public int delete(final Uri uri, final String selection, final String[] selectionArgs) {
		...
		case FAVORIS:
			count = db.delete(StopBookmarkTable.TABLE_NAME, selection, selectionArgs);
			break;
		case FAVORI_ID:
			final String segment = uri.getLastPathSegment();
			count = db.delete(StopBookmarkTable.TABLE_NAME, StopBookmarkTable._ID + "=" + segment
	...
	public Uri insert(final Uri uri, final ContentValues initialValues) {
		...
		final SQLiteDatabase db = getWritableDatabase();
		final long rowId = db.insert(StopBookmarkTable.TABLE_NAME, null, values);
		if (rowId > 0) {
	...
	public Cursor query(final Uri uri, String[] projection, final String selection, final String[] selectionArgs,
		...
		queryBuilder.setTables(StopBookmarkTable.TABLE_NAME);

		final int uriType = URI_MATCHER.match(uri);
		switch (uriType) {
		case FAVORI_ID:
			queryBuilder.appendWhere(StopBookmarkTable._ID + "=" + uri.getLastPathSegment());
			break;
		case FAVORIS_FULL:
			queryBuilder.setTables(StopBookmarkTable.JOIN);
			projection = StopBookmarkTable.FULL_PROJECTION;
			if (sortOrder == null) {
				sortOrder = StopBookmarkTable.FULL_ORDER;
			}
		case FAVORIS:
	...
	public int update(final Uri uri, final ContentValues values, final String selection, final String[] selectionArgs) {
		final SQLiteDatabase db = getWritableDatabase();
		final int rowCount = db.update(StopBookmarkTable.TABLE_NAME, values, selection, selectionArgs);
		if (rowCount > 0) {
			getContext().getContentResolver().notifyChange(uri, null);
		}
		return rowCount;

...
public class BiclooBookmarkProvider extends CustomContentProvider {

	public static final int FAVORIS = 100;
	public static final int FAVORI_ID = 110;

	private static final String AUTHORITY = "net.naonedbus.provider.BiclooBookmarkProvider";
	...
	public int delete(final Uri uri, final String selection, final String[] selectionArgs) {
		...
		case FAVORIS:
			count = db.delete(BiclooBookmarkTable.TABLE_NAME, selection, selectionArgs);
			break;
		case FAVORI_ID:
			final String segment = uri.getLastPathSegment();
			count = db.delete(BiclooBookmarkTable.TABLE_NAME,
					BiclooBookmarkTable._ID + "=" + segment
	...
	public Uri insert(final Uri uri, final ContentValues initialValues) {
		...
		final SQLiteDatabase db = getWritableDatabase();
		final long rowId = db.insert(BiclooBookmarkTable.TABLE_NAME, null, values);
		if (rowId > 0) {
	...
	public Cursor query(final Uri uri, final String[] projection, final String selection, final String[] selectionArgs,
		...
		queryBuilder.setTables(BiclooBookmarkTable.TABLE_NAME);
		if (sortOrder == null) {
			sortOrder = BiclooBookmarkTable.EQUIPMENT_NAME;
		}

		final int uriType = URI_MATCHER.match(uri);
		switch (uriType) {
		case FAVORI_ID:
			queryBuilder.appendWhere(BiclooBookmarkTable._ID + "=" + uri.getLastPathSegment());
			break;
		case FAVORIS:
	...
	public int update(final Uri uri, final ContentValues values, final String selection, final String[] selectionArgs) {
		final SQLiteDatabase db = getWritableDatabase();
		final int rowCount = db.update(BiclooBookmarkTable.TABLE_NAME, values, selection, selectionArgs);
		if (rowCount > 0) {
			getContext().getContentResolver().notifyChange(uri, null);
		}
		return rowCount;

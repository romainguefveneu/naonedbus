...
import net.naonedbus.activity.map.overlay.MargueriteItemizedOverlay;
 ...
 * 
 */
public class MargueriteMapLayer extends EquipementMapLayer {

	public MargueriteMapLayer() {
		super(Equipement.Type.TYPE_MARGUERITE, TypeOverlayItem.TYPE_MARGUERITE);
	}

	@Override
	protected BasicItemizedOverlay getOverlay(Resources resources) {
		return new MargueriteItemizedOverlay(resources);
	}
	...
	public ItemSelectedInfo getItemInfo(final Context context, final BasicOverlayItem item) {
		return new EquipementSelectedInfo() {
			...
			public Integer getResourceDrawable() {
				return R.drawable.map_layer_marguerite;
			}

			@Override
			public Integer getResourceColor() {
				return R.color.map_pin_marguerite;
			}
			...
			public String getDescription(final Context context) {
				return context.getString(R.string.map_calque_marguerite);
			}
